...
extern "C" {
     ...
     * @param url           A url identifying the eye tracker. Example: "tet-tcp://172.68.195.1".
    ...
    /**
     * Sets the logging file name and verbosity.
    ...
    /**
     * Enables or disables logging to standard output and sets the logging verbosity.
     * @param is_enabled            Indicates whether logging to standard output should be enabled or not. Zero means disabled.
     * @param log_level             The verbosity of the logging.
     * @param error_code            Will be set to TOBIIGAZE_ERROR_SUCCESS if operation was successful, otherwise to an error code. Can be NULL.
     */
    TOBIIGAZE_API void TOBIIGAZE_CALL tobiigaze_set_standard_output_logging(int is_enabled, tobiigaze_log_level log_level, tobiigaze_error_code *error_code);
    ...
    /**
     * Starts an internal thread and runs the event loop on it.
     * @param eye_tracker   An eye tracker instance.
    ...
    /**
     * Breaks the event loop. This will make the blocking run_event_loop call return.
     * After this function has been called, it is NOT possible to start a new event loop on the same eye tracker instance.
     * This function must NOT be called from a callback. Any outstanding work is cancelled.
     * If run_event_loop was used to start the loop, it is up to the client to synchronize the thread with join().
     * If run_event_loop_on_internal_thread was used, the thread is joined in this call.
     * @param eye_tracker   An eye tracker instance.
     */
