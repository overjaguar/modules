...
Building on Windows
-------------------
The sample code includes a project file for Visual Studio 2012.

1. Download and install wxWidgets version 3.0 or later from
   https://www.wxwidgets.org/downloads/. Make sure that you have set the WXWIN 
   environment variable, as described in the instructions. We recommend using the 
   source code package, since it is easier to get started with than the binaries 
   packages.
...
3. Build the wxWidgets library according to the instructions in
   %WXROOT%\docs\msw\install.txt. Make sure to use the VS2012 command prompt with
   correct platform when building wxWidgets.
