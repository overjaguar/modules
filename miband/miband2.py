...
class AuthenticationDelegate(DefaultDelegate):
    ...
    def __init__(self, device):
        ...
        if not os.path.exists('log'):
            os.mkdir('log')
        filename = datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S.MiBand2.csv")
        self.f = open('./log/' + filename, 'w')
        self.f.write(header + '\n') 
    ...
    def handleNotification(self, hnd, data):
        ...
        if hnd == self.device.char_auth.getHandle():
            ...
            heart = str(rate)
            print("Heart Rate: " + heart)
            self.f.write(datetime.datetime.now().strftime("%H:%M:%S") + ";" + heart + "\n")
            # print >>self.f, str(rate), "\n",
        else:
...
def main():
    ...
    parser.add_argument('-t', action='store_true', default=False, help='Start testing.')
    parser.add_argument('-a', action='store_true', default=False, help='Preinitialize of MiBand.')
    
    arg = parser.parse_args(sys.argv[1:])

    if not arg.a and not arg.t:
        print('Missing key -a or -t. Close')
        return

    if arg.a and arg.t:
        print('Choise one of arguments: -a or -t. Close')
        return

    f = open('mac_key.txt', 'r')
    mac_key = None  
    for line in f:
        mac_key = line    
        break
    ...
    f.close()
    mac_key = mac_key.split()[0]

    print('Connecting to ' + mac_key)

    band = MiBand2(mac_key)
    ...
    print("Press ENTER to stop(duration 20 sec)")
