...
typedef enum
{
    ...
    TOBIIGAZE_STREAM_TYPE_NAME_MAX_SIZE = 64,
    TOBIIGAZE_EXTENSION_NAME_MAX_SIZE = 16,
    TOBIIGAZE_EXTENSIONS_MAX_SIZE = 16,
    TOBIIGAZE_STREAM_TYPES_MAX_SIZE = 16,
...
/**
 * This structs contains for big blob-like data. @field data
 * must be allocated by the client.
 */
...
/**
 * This struct holds viedo data.
 */
struct tobiigaze_video_data
{
    uint64_t timestamp;
...
struct tobiigaze_stream_type
{
    uint32_t id;
    char name[TOBIIGAZE_STREAM_TYPE_NAME_MAX_SIZE];
    char description[TOBIIGAZE_STREAM_TYPE_NAME_MAX_SIZE];
    uint32_t realm;
};

struct tobiigaze_stream_types
{
    struct tobiigaze_stream_type stream_types[TOBIIGAZE_STREAM_TYPES_MAX_SIZE];
    uint32_t actual_size;
};
